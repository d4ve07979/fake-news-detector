const NAMES = { home:"Accueil", analyzer:"Analyser un article", dashboard:"Tableau de bord", history:"Historique", about:"À propos" };
const IconMenu = () => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="18" x2="21" y2="18"/></svg>;
const IconSun  = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg>;
const IconMoon = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>;

export default function Topbar({ page, theme, toggleTheme, onMenuClick, onMobileMenu }) {
  return (
    <header className="topbar">
      <button className="topbar-menu-btn" onClick={onMenuClick || onMobileMenu} style={{ display:"flex" }}>
        <IconMenu />
      </button>
      <div className="topbar-breadcrumb">
        <span>FakeDetect</span>
        <span className="sep">/</span>
        <span>{NAMES[page] || page}</span>
      </div>
      <div className="topbar-actions">
        <button className="theme-toggle" onClick={toggleTheme}>
          {theme === "dark" ? <IconSun /> : <IconMoon />}
          <span>{theme === "dark" ? "Mode clair" : "Mode sombre"}</span>
        </button>
        <div className="topbar-avatar" title="KENKOU Dave">KD</div>
      </div>
    </header>
  );
}
